...
class RoverState():
    def __init__(self):
        ...
        self.max_vel = 1.25 # Maximum velocity (meters/second)
        ...
        self.destination = np.float32([[160 - dst_size, 160 - bottom_offset],
...
def telemetry(sid, data):
    ...
    frame_counter+=1
    # Do a rough calculation of frames per second (FPS), once every 10 seconds
    if (time.time() - second_counter) > 10:
        fps = frame_counter
        frame_counter = 0
        second_counter = time.time()
        print("Current FPS: {}".format(fps/10))
